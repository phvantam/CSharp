...
-- Kiểm tra UserProfile đã được tạo chưa
...
INSERT INTO Artists (Name, Slug, Bio, IsVerified, CreatedAt, UpdatedAt)
VALUES 
('STRONG', 'Vpop', N'Nghệ sĩ chưa xác định', 0, GETUTCDATE(), GETUTCDATE());
SELECT 
    Id,
    UserName,
    NormalizedUserName,
    ...
    PasswordHash
FROM AspNetUsers
WHERE Email = 'tampham@gmail.com'
   OR UserName = 'tampham';

   SELECT Id, UserName, Email, PasswordHash
FROM AspNetUsers;
SELECT TOP 10 * FROM MediaItems ORDER BY CreatedAt DESC
SELECT 
    COLUMN_NAME, 
    DATA_TYPE, 
    IS_NULLABLE
...
SELECT COUNT(*) AS TotalMediaItems FROM MediaItems;
SELECT 
    MediaItemId,
    Title,
    MediaType,
...
DBCC CHECKIDENT ('MediaItems', RESEED, 0);
...
SELECT 
    ...
    CreatedAt
FROM MediaItems
WHERE Title LIKE N'%TẤM LÒNG CỬU LONG%'
ORDER BY MediaItemId DESC;
DELETE FROM MediaItems
WHERE Title LIKE N'%TẤM LÒNG CỬU LONG%';
...
(
    SELECT
        MediaShareId,
        ROW_NUMBER() OVER (
            PARTITION BY SenderUserId, ReceiverUserId, MediaItemId, PlaylistId, ShareType
            ORDER BY CreatedAt DESC, MediaShareId DESC
        ) AS rn
    FROM MediaShares
    WHERE IsRevoked = 0
)
DELETE FROM MediaShares
WHERE MediaShareId IN (
    SELECT MediaShareId
    FROM Duplicates
    WHERE rn > 1
);
...
SELECT PlaylistId, Title, CoverImageUrl, UpdatedAt
FROM Playlists
...
SELECT 
    ...
    h.LastPlayedAt,
    h.ProgressSeconds
FROM dbo.PlayHistories h
LEFT JOIN dbo.MediaItems m ON h.MediaItemId = m.MediaItemId
WHERE EXISTS (
    SELECT 1
    FROM dbo.PlayHistories h2
    WHERE h2.UserId = h.UserId
      AND h2.MediaItemId = h.MediaItemId
    GROUP BY h2.UserId, h2.MediaItemId
    HAVING COUNT(*) > 1
)
ORDER BY h.UserId, h.MediaItemId, h.LastPlayedAt DESC;

WITH DuplicateHistory AS
(
    SELECT 
        PlayHistoryId,
        ROW_NUMBER() OVER (
            PARTITION BY UserId, MediaItemId
            ORDER BY LastPlayedAt DESC, PlayHistoryId DESC
        ) AS rn
    FROM dbo.PlayHistories
)
DELETE FROM dbo.PlayHistories
WHERE PlayHistoryId IN (
...
SELECT * FROM dbo.MediaItems ORDER BY MediaItemId DESC;
SELECT * FROM dbo.Artists ORDER BY ArtistId DESC;
SELECT * FROM dbo.Albums ORDER BY AlbumId DESC;
SELECT * FROM dbo.Playlists ORDER BY PlaylistId DESC;
SELECT * FROM dbo.Favorites ORDER BY CreatedAt DESC;
SELECT * FROM dbo.PlayHistories ORDER BY LastPlayedAt DESC;

-- Cập nhật Genre cho các bài hát cũ đang NULL trong TuneVault
-- Chạy trong SQL Server trên database TuneVaultDB

UPDATE dbo.MediaItems SET Genre = N'Ballad'
WHERE Title IN (
    N'Nơi Này Có Anh',
    N'Sau Tất Cả',
    N'Có Hẹn Với Thanh Xuân',
    N'Em Thua Cô Ta',
    N'Thiệp Hồng Sai Tên',
    N'Có Chàng Trai Viết Lên Cây'
)
AND (Genre IS NULL OR LTRIM(RTRIM(Genre)) = N'');

UPDATE dbo.MediaItems SET Genre = N'Rap / Hip-hop'
WHERE Title IN (
    N'Mang Tiền Về Cho Mẹ',
    N'Không Thể Say'
)
AND (Genre IS NULL OR LTRIM(RTRIM(Genre)) = N'');

UPDATE dbo.MediaItems SET Genre = N'EDM / Dance'
WHERE Title IN (
    N'See Tình'
)
AND (Genre IS NULL OR LTRIM(RTRIM(Genre)) = N'');

UPDATE dbo.MediaItems SET Genre = N'Indie'
WHERE Title IN (
    N'Lạ Lùng'
)
AND (Genre IS NULL OR LTRIM(RTRIM(Genre)) = N'');

UPDATE dbo.MediaItems SET Genre = N'V-Pop'
WHERE Title IN (
    N'Waiting For You',
    N'Come My Way'
)
...
UPDATE dbo.MediaItems SET Genre = N'Nhạc trẻ'
WHERE Genre IS NULL OR LTRIM(RTRIM(Genre)) = N'';
...
SELECT 
    ...
    MediaType
FROM dbo.MediaItems
WHERE Title = N'MASHUP ROCK THIỆP HỒNG';
...
UPDATE dbo.[MediaItems]
SET Genre = N'OST', UpdatedAt = SYSDATETIME()
WHERE Title IN (N'Not The Best But Still Good');
SELECT MediaItemId, Title, Genre, CreatedAt
FROM dbo.MediaItems
ORDER BY MediaItemId;

SELECT MediaItemId, Title, DurationSeconds, AudioFilePath, VideoFilePath
FROM dbo.MediaItems
...
SELECT 
    ...
    ma.Position
FROM MediaItems m
LEFT JOIN MediaArtists ma ON ma.MediaItemId = m.MediaItemId
LEFT JOIN Artists a ON a.ArtistId = ma.ArtistId
WHERE m.MediaItemId =31 -- nhập id bài MASHUP ROCK THIỆP HỒNG
